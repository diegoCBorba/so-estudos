...
def menor_processo(dici):
    """
    Recebe o dicionário referente aos processos e retorna uma string referente ao processo com o menor
    tempo de execução.
    """

    p_menor = list(dici.keys())[0]
    for processo in dici:
        if dici[p_menor][0] > dici[processo][0]:
            p_menor = processo
...
# para ser trabalhada no código do escalonamento SJF
...
while len(dic_processos) > 0:
    lista_removidos = []
    p_atual = menor_processo(dic_processos)
    print("╭╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╮")
